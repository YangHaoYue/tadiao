...
var _detail = _interopRequireDefault(__webpack_require__(/*! ./pages/mine/projectManagement/detail/detail.vue */ 121));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}
...
/* harmony import */ var _detail_vue_vue_type_template_id_5cdcb10e_scoped_true___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./detail.vue?vue&type=template&id=5cdcb10e&scoped=true& */ 122);
...
/* harmony import */ var _detail_vue_vue_type_style_index_0_id_5cdcb10e_scoped_true_lang_css___WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./detail.vue?vue&type=style&index=0&id=5cdcb10e&scoped=true&lang=css& */ 126);
...

{
  ...
  onLoad: function onLoad(e) {var _this = this;
    ...
    this.orderPays();
    this.getInfo();
    uni.$on('addRecordback', function (res) {
      _this.payList.order_pays_data = [];
      _this.payList.current_page = 1;
      _this.orderPays();
    });
  },
  ...
  methods: {
    getInfo: function getInfo() {var _this2 = this;
      ...
      then(function (res) {
        if (res.code == 1000) {
          _this2.contract.contract_imgs = res.data.contract_imgs.map(function (v) {
            return _this2.http.resourceUrl() + v;
          });
          _this2.contract.contract_num = res.data.contract_num;

          _this2.list[0].list[0].value = res.data.cus.name;
          _this2.list[0].list[0].avatar = _this2.http.resourceUrl() + res.data.cus.avatar;
          _this2.list[0].list[1].value = res.data.cus.real_name;
          _this2.list[0].list[2].value = res.data.cus.tel_num;
          _this2.list[0].list[3].value = res.data.cus.address;

          _this2.list[1].list[0].value = res.data.executor.name;
          _this2.list[1].list[0].avatar = _this2.http.resourceUrl() + res.data.executor.avatar;
          _this2.list[1].list[1].value = res.data.executor.real_name;
          _this2.list[1].list[2].value = res.data.executor.tel_num;

          _this2.list[2].list[0].value = res.data.handler.name;
          _this2.list[2].list[0].avatar = _this2.http.resourceUrl() + res.data.handler.avatar;
          _this2.list[2].list[1].value = res.data.handler.real_name;
          _this2.list[2].list[2].value = res.data.handler.tel_num;

          _this2.list[3].list[0].value = res.data.provider.name;
          _this2.list[3].list[0].avatar = _this2.http.resourceUrl() + res.data.provider.avatar;
          _this2.list[3].list[1].value = res.data.provider.real_name;
          _this2.list[3].list[2].value = res.data.provider.tel_num;

          _this2.equipList = res.data.towers;
          _this2.equipList.tower_data.forEach(function (v) {
            _this2.$set(v, 'maintainList', '');
            _this2.$set(v, 'repairList', '');
            _this2.towerCares(v.id, v);
            _this2.towerFixes(v.id, v);
          });
          console.log(_this2.equipList);
          _this2.fix_comment = res.data.fix_comment;
          _this2.tower_comment = res.data.tower_comment;

          _this2.show_close_button = res.data.show_close_button;
          _this2.show_order_pay_button = res.data.show_order_pay_button;

          _this2.orderList[0].value = res.data.order_no;
          _this2.orderList[1].value = res.data.created_at;
          _this2.orderList[2].value = res.data.lease_start_at;
          _this2.orderList[3].value = res.data.lease_end_at;
          _this2.orderList[4].value = res.data.type_pay == 1 ? '季结' : '月结'; //0=>月结,1=>季结

          _this2.totalPrice = res.data.total_amount;
        }
      });
    },
    //保养记录
    towerCares: function towerCares(id, item) {var _this3 = this;
      ...
      this.http.get('Order/towerCares', {
        ...
        if (!item.maintainList) {
          _this3.$set(item, 'maintainList', res.data);
        } else {
    ...
    //维修记录
    towerFixes: function towerFixes(id, item) {var _this4 = this;
      ...
      this.http.get('Order/towerFixes', {
        ...
        if (!item.repairList) {
          _this4.$set(item, 'repairList', res.data);
        } else {
    ...
    //获取收款记录
    orderPays: function orderPays() {var _this5 = this;
      if (this.payList.current_page > this.payList.last_page) return;
      this.http.get('Order/orderPays', {
        order_id: this.order_id,
        page: this.payList.current_page || 1 }, true).then(function (res) {
        if (!_this5.payList.order_pays_data) {
          _this5.$set(_this5, 'payList', res.data);
        } else {
          res.data.order_pays_data.map(function (v) {
            _this5.payList.order_pays_data.push(v);
          });
        }
        _this5.payList.current_page++;
      });
    },
    Collection: function Collection(id) {var _this6 = this;
      this.http.modal('', '是否确认收款', true, function (res) {
        if (res) {
          _this6.http.post('Order/orderPayCollect', {
            order_pay_id: id }).
          then(function (res) {
            _this6.$u.toast(res.msg);
            if (res.code == 1000) {
              _this6.payList.order_pays_data = [];
              _this6.payList.current_page = 1;
              _this6.orderPays();
            }
          });
        }
    ...
    end: function end() {var _this7 = this;
      this.http.modal('', '是否要结束该项目？', true, function (res) {
        if (res) {
          _this7.http.post('Order/closeOrder', {
    ...
    toAddRecord: function toAddRecord(id) {
      uni.navigateTo({ url: "../addRecord/addRecord?order_id=".concat(this.order_id) });
    },
    //预览图片
