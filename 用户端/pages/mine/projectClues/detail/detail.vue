		...
		<view class="u-m-25 card">
			...
			<view class="u-p-20">
				<view class="u-flex u-row-between u-col-top u-p-10" v-for="(item,index) in list1" :key="index">
					<view class="u-font-28" style="color: #999999;min-width: 200rpx;">{{item.name}}</view>
					<view class="u-font-28 text-bold u-text-right" style="color: #333333;" v-if="item.value.constructor == String">{{item.value}}</view>
					<view class="u-font-28 text-bold u-text-right" style="color: #333333;" v-else>
						<view v-for="(son,i) in item.value" :key="i" >{{son.type}}({{son.count}}台)</view>
					</view>
				</view>
				<block v-for="(item,i) in medias" :key="'i'+i">
					<view class="u-flex u-row-between u-col-top u-p-10" >
						<view class="u-font-28" style="color: #999999;">联系人{{i+1}}</view>
						<view class="u-font-28 text-bold" style="color: #333333;">{{item.media_name}}</view>
					</view>
					<view class="u-flex u-row-between u-col-top u-p-10" >
						<view class="u-font-28" style="color: #999999;">联系电话{{i+1}}</view>
						<view class="u-font-28 text-bold" style="color: #333333;">{{item.media_tel_num}}</view>
					</view>
				</block>
				<view class="u-flex u-row-between u-col-top u-p-10" v-for="(item,n) in list2" :key="'n'+n">
						...
						<template v-slot:content>
							<view>
								...
								<view class="u-flex u-flex-wrap">
									<block class="u-p-10" v-for="(son,k) in item.img" :key="'k'+k">
